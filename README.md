...
* `mgt-deploy.sh` - Deployment script for Management Pattern
* `mgt-teardown.sh` - Tear down script for Management Pattern
* `ns-mgt.yml` - Management Pattern Namespace
* `rc-mgt-prometheus.yml` - Prometheus deployment manifest
* `svc-mgt-prometheus.yml` - Prometheus service definition

Make `mgt-deploy.sh` and `mgt-teardown.sh` executable.

* `chmod +x mgt-teardown.sh`
* `chmod +x mgt-deploy.sh`

To install run `mgt-deploy.sh` and to uninstall run `mgt-teardown.sh`.

Once deployed use `kubectl get --namespace=management ns,pods,ds,rs,rc,svc' to determine the status of the components.

The UI for Prometheus can be accessed on the Public IP of the `http://<Public IP>:9090`
